...
describe('loadSettings', () => {
  ...
  it('round-trips valid settings saved via saveSettings', () => {
    saveSettings({ colorTheme: 'dark', gridSize: 4, defaultSearchFilters: DEFAULT_SEARCH_FILTERS })
    expect(loadSettings()).toEqual({ colorTheme: 'dark', gridSize: 4, defaultSearchFilters: DEFAULT_SEARCH_FILTERS })
  })
  ...
  it('keeps a stored, valid default search filter as-is', () => {
    ...
    expect(loadSettings()).toEqual({ colorTheme: 'dark', gridSize: 4, defaultSearchFilters: customFilters })
  })
})
...
describe('loadSettingsOrDefault', () => {
  ...
  it('prefers stored settings over the system default', () => {
    mockMatchMedia(true)
    saveSettings({ colorTheme: 'light', gridSize: 2, defaultSearchFilters: DEFAULT_SEARCH_FILTERS })
    expect(loadSettingsOrDefault()).toEqual({
      colorTheme: 'light',
      gridSize: 2,
      defaultSearchFilters: DEFAULT_SEARCH_FILTERS,
